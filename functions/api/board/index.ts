...
import { MAX_POST_LEN, cleanBoardText, parseBefore, parseLimit, parsePositiveInt } from '../../../src/server/board'
...
/** 列表里每帖最多带几条回复预览（点开再看全部，避免一屏拉几千行） */
const PREVIEW_REPLIES = 3

/** 按 reply id 倒序分页拉取某帖的更多回复（before=0 表示从最新开始）。 */
async function fetchReplies(env: Env, postId: number, before: number, limit: number): Promise<BoardReplyDto[]> {
...
async function fetchPreviewReplies(env: Env, postIds: number[], preview: number): Promise<Map<number, BoardReplyDto[]>> {
...
async function fetchReplyCounts(env: Env, postIds: number[]): Promise<Map<number, number>> {
...
/**
 * 读留言板。**不需要登录**——没登录也该看得见大家在说什么，
 * 只是发帖/回复要登录（见 POST 与各删除接口）。
 */
...
export const onRequestPost = handle(
  requireSession(async (context) => {
    ...
    if (!content) {
      return json({ error: { code: 'invalid_content', message: `内容不能为空且不超过 ${MAX_POST_LEN} 字` } }, 400)
    }
