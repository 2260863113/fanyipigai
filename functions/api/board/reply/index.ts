...
import { MAX_REPLY_LEN, cleanBoardText, parsePositiveInt } from '../../../../src/server/board'
...
export const onRequestPost = handle(
  requireSession(async (context) => {
    ...
    if (!content) {
      return json({ error: { code: 'invalid_content', message: `回复不能为空且不超过 ${MAX_REPLY_LEN} 字` } }, 400)
    }
