/**
 * 会话 token 的签发与校验。token 明文只出现在响应体与浏览器里；库里只存 SHA-256 摘要。
 *
 * 复用自「地图记忆」的 `functions/_lib/auth.ts`，只去掉了那边 users 表的 `hometown` 列。
 */

import { bearer, type Env } from './http'

/** 会话有效期 30 天：练一次翻译不该每次重新登录 */
...
/** 生成会话并返回明文 token（明文只在这一刻存在，之后库里只有摘要）。 */
