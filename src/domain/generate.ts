 ...
 * 出题时可选的领域（也可以自己输入）。
 *
 * 就是文章库那**五个板块**：社会、经济、文化、生态、科技。
 * 为什么不再照抄赛制的"五位一体"＋三个延伸（共八个）：站里的领域表已经收敛到这五个，
 * 两处口径不一致会让用户看到两个不同的领域清单（见 ADR 0010）。
 * 不往这几个之外列（娱乐、体育、个人生活之类），免得把学生往偏题上引。
 */
export const GENERATION_TOPICS: readonly string[] = ['社会', '经济', '文化', '生态', '科技']
