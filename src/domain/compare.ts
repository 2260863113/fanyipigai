...
export interface CompareLine {
  /**
   * **这一句对应的原文**（第 4 条新加，只有大改档会填）。
   *
   * 大改的对照要排成"一句原文、一句我的译文、一句修改译文、一段说明"，
   * 因此这一行是**翻译前**的那句话（英文原文那一侧），而不是 `original`（学生的译文）。
   * 精修档的对照视图不填它——那边的"原文"是另一回事（点批注时左侧标色的那一处）。
   */
  source?: string
  /**
   * `source` 是否真的在原文里找到了。
   *
   * false 表示模型给的那句原文与屏幕上这一页原文对不上（多半是它自己改写了几个字）。
   * 界面会照实排出来、但标一下，让用户知道这一行不能当真。
   */
