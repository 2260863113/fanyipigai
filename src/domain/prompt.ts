...
## 批改方法：**分四遍自查，每遍只盯一类错误**（用户指定，务必照做）

不要一次把整段看一遍就出结果。**按下面的顺序走四遍**，每一遍**只找这一类的错**，
找完再进下一遍；四遍都走完之后再一起输出。

| 第几遍 | 这一遍只看什么 | 用哪些分类 | 颜色 |
| --- | --- | --- | --- |
| 第 1 遍 | **语法**：冠词用对没有、时态、名词单复数、介词、主谓一致、句子结构是否成立 | grammar / function-word | 红 |
| | 中文译文的标点有没有用全角 | punctuation | 红 |
| 第 2 遍 | **漏译与多译**：原文有的信息译文里有没有；译文里有没有原文没有的信息 | omission / addition | 由程序按字数定 |
| 第 3 遍 | **专有名词与术语**：固定表述、关键术语、专有名词是否与官方译法**一字不差** | terminology | 红 |
| 第 4 遍 | **表达是否地道**：生硬、别扭、搭配不成立、语序需要调整、语体与文体不匹配 | word-choice / collocation / register / word-order | 橙 |

**只有第 4 遍才给 highlights（亮点）**：前三遍是在挑错，正面的判断放到最后一遍一起说。

### 一趟错误只归一类，按优先级取靠前的那一类

上面四类错误的优先级**从高到低**就是 1 → 2 → 3 → 4：
**同一处文字同时像两类（或更多类）错误时，只算优先级高的那一类，不要报两遍**
（一处错误只归一类，否则用户看到的弱项统计会变成同义词碎片）。

- 一句话里既有语法错、又读着别扭 → **只报语法（红）**，不要再报"用词不当"；
- 术语写错、句子同时也不通 → 那是**两处不同的字**，各自照各自那一遍报；
- **不要因为"读着别扭、不够地道"就把语法错误写成 word-choice（橙）**——
  这是最常犯的错，会让用户以为自己只是用词不讲究，其实句子根本不通。

### 第 2 遍的两条硬性要求（程序要靠它算轻重，务必照做）

漏译与多译的**红/橙由程序按字数判**（漏得少算橙、漏得多算红），程序要能量到字数，
这两种错就**必须长成固定的形状**：
...
export function buildRefineRetryPrompt(problems: string[]): string {
  return [
    ...
    ``,
    `请重新返回**完整的 JSON**（不是只返回改动的部分），逐条修正上面的问题。特别注意：`,
    `- 顶层三个字段一个都不能少：score（0–100 的整数）、comment、sentences`,
    `- sentences 里**每一句都要有一条**，包括你一个字都没改的句子（rewritten 照抄原句）；`,
    `- 每条都要有 original、rewritten、explanation 三个字段，一个都不能少；`,
    `- **original 必须与学生译文里的那一句逐字相同**（含标点与空格）。` +
      `程序会拿这段文字去译文里找位置，改一个字就找不到，那一句就画不出来。`,
    `- explanation 每一小点之间用全角分号「；」隔开。`,
  ].join('\n')
}
 ...
 *   3. 最后给一个**总体分数与解释**（"不统计，不逐处批改，但是让 AI 给一个最终分数和解释"）。
 */
...
- comment：为什么是这个分数；两三个小点，**每一小点之间用全角分号「；」隔开**。

## 返回格式
只返回一个 JSON 对象，不要包裹代码块，不要加任何解释文字。**不要输出任何字符序号。**
{
  "score": 88,
  "comment": "第一小点；第二小点；第三小点。",
  "sentences": [
    {
